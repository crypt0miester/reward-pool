...
// src/farm.ts
import { BN, EventParser as EventParser2 } from "@coral-xyz/anchor";
...
var FARM_PROGRAM_ID = new PublicKey(
  "FarmuwXPWXvefWUeqFAa5w6rifLkq5X6E8bimYvrhCB1"
);
var AMM_PROGRAM_ID = new PublicKey(
...
var PoolFarmImpl = class _PoolFarmImpl {
  ...
  static create(connection, farm, opt) {
    return __async(this, null, function* () {
      ...
      const eventParser = new EventParser2(FARM_PROGRAM_ID, program.coder);
  ...
  static getFarmAddressesByPoolAddress(poolAddress, cluster) {
    return __async(this, null, function* () {
      ...
      return farms.map((farm) => ({
        farmAddress: new PublicKey3(farm.farming_pool),
  ...
  static getUserBalances(connection, owner, farmMints) {
    return __async(this, null, function* () {
      ...
      const userStakingPda = farmMints.map((mint) => {
        const [userStakingAddress] = PublicKey3.findProgramAddressSync(
    ...
    return __async(this, null, function* () {
      ...
      return Promise.all(
        chunkedClaimAllTx.map((claimAllTx) => __async(this, null, function* () {
          ...
          }).add(ComputeBudgetProgram.setComputeUnitLimit({ units: 14e5 })).add(...claimAllTx);
        }))
      );
    });
  }
  ...
  getUserPda(owner) {
    const [userPda] = PublicKey3.findProgramAddressSync(
      [owner.toBuffer(), this.address.toBuffer()],
  ...
  deposit(owner, amount) {
    return __async(this, null, function* () {
      ...
      return new Transaction(__spreadValues({
        feePayer: owner
    ...
    return __async(this, null, function* () {
      ...
      return Array.from(poolStatesMap.entries()).reduce((accValue, [farmMint, { poolState, userState }]) => {
        ...
        if (!userState || !poolState)
          return accValue;
        const rewardA = userState.balanceStaked.mul(a.sub(userState.rewardAPerTokenComplete)).div(new BN(1e9)).add(userState.rewardAPerTokenPending);
        const rewardB = userState.balanceStaked.mul(b.sub(userState.rewardBPerTokenComplete)).div(new BN(1e9)).add(userState.rewardBPerTokenPending);
